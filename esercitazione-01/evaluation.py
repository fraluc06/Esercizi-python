...
def evaluate(funct, is_assert=True):
    ...
    if fname in evaluation_tests:
        ...
        for i, (inp, ex) in enumerate(zip(inpts, expts), 1):
            ...
            if out is None: 
                print(f'{fname} is not implemented')
                return
            if is_assert:
            ...
            print(f'{"="*50}\n>Test {i} of {fname}{inp!r} PASSED! with expected value {out!r}')
            if in_place:
                print(f'>Also, input {inp!r} was modified CORRECTLY as {exp_in!r} ')

    else:
        print((f'> La funzione {fname} non è nei casi di test, controlla il nome della funzione'))
...
def show_tests(funct):
    ...
    if fname in evaluation_tests:
        ...
        for count, (inp, exp) in enumerate(zip(inpts, expts)):
            print(f'{"="*50}\n> {count})CHIAMATA: {fname}{inp!r}')
            if in_place:
                exp_in = next(iter_expts_in)
                print(f'> {count})ARGOMENTI MODIFICATI ATTESI: {exp_in!r}')
            print(f'> {count})RISULTATO ATTESO: {exp!r}', end='\n\n')
