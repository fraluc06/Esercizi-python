...
def num_digits(intero):
    str_intero = str(intero)
    return len(str_intero)
    # inserisci qui il tuo codice
...
def num_digits_neg_str_mix(v):
    return num_digits_neg(v)    # inserisci qui il tuo codice
...
def check_int_str(stringa, intero):
    ...
    str_intero =str(intero)
    sub_stringa = stringa
    while sub_stringa.find(str_intero) != -1:
        pos = sub_stringa.find(str_intero)
        count += 1
        sub_stringa = sub_stringa[pos + len(str_intero):]
    return count
...
def count_sub_string(query, corpo):
    ...
    # sub_stringa = corpo
    # while sub_stringa.find(query) != -1:
    #     pos = sub_stringa.find(query)
    #     count += 1
    #     sub_stringa = sub_stringa[pos + 1:]
    # return count
    # inserisci qui il tuo codice
...
def check_reverse(S, T):
    ...
    # string_S = str(S)
    # string_T = str(T)
    # if string_S == string_T[::-1]:  # trasformo la stringa in modo da essere scritta al rovescio
    #     return True
    # else:
    #     return False
    # inserisci qui il tuo codice
