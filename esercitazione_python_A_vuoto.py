...
#
# %% evaluation
from evaluation import evaluate, show_tests
...
def num_digits(intero):
    intero_str = str(abs(intero))
    return len(intero_str)
...
def check_S_in_T(S, T):
    ...
    #     # print(f" {S[i]}")
    #     int_S = int(S[i])
    #     count = check_int_str(T, int_S)
    #     L.append(count)
        # print(L)
    L = [T.count(c) for c in S]
    return L
...
def count_sub_string_idx(query, corpo):
    ...
    while i <= len(corpo)-1:
        pos = corpo.find(query,i)
        if pos !=-1:     #trovato query in corpo alla posizione pos
            i = pos+1
            T = (pos,pos +len(query)-1)
            L.append(T)
        else: i+=1
    return L
    # inserisci qui il tuo codice
...
all' elemento i-esimo la somma accumulata degli elementi della lista L
...
def get_list_except_min_max_general(L):
    ...
    L_min = min(L)
    L_max = max(L)
    L_count_min = L.count(L_min)
    L_count_max = L.count(L_max)
    for i in range(0, L_count_min):
        L.remove(L_min) 
    if (L_min != L_max):
        for i in range(0, L_count_max):
            L.remove(L_max)
        return L_count_min + L_count_max
    else:
        return L_count_min
...
def list_to_tuple(L):
    ...
    for i in L:
       # Inverti la tupla corrente
       tupla_inv = list(i[::-1])
       
       # Calcola il prodotto della tupla
       prodotto = 1
       for valore in i:
           prodotto *= valore
       
       # Controlla se il prodotto è multiplo di 2
       if prodotto % 2 == 0:
           res.append(tupla_inv)
   
   # Restituisci il risultato come tupla di liste
    return tuple(res)
...
def sort_by_str(T): 
    ...
    L = list(T)
    L.sort(key=myFunc)
    cur_len, prev_len, cur_row = 0, 0, -1
    for i in range(0, len(L)):
        cur_len = len(L[i])
        if (cur_len>prev_len):
            M.append([L[i]])
...
def sort_by_month(L):
    ordine_mesi = ["gennaio","febbraio","marzo","aprile","maggio","giugno",
                   ...
                   "novembre","dicembre"]
    return sorted(L, key=lambda x: ordine_mesi.index(x.lower()))
...
def int_to_hist(values):
    # Trova il minimo e il massimo valore nella lista
   min_val, max_val = min(values), max(values)
   
   # Calcola la frequenza di ogni valore in 'values'
   frequency = {i: 0 for i in range(min_val, max_val + 1)}
   for value in values:
       frequency[value] += 1
   
   # Genera la stringa dell'istogramma
   histogram = ""
...
def anagramma(S : str, T: str) -> bool:
    # Rimuoviamo eventuali spazi per gestire casi come 'a gentleman' e 'elegant man'
    S = S.replace(" ", "")
    T = T.replace(" ", "")
    
    # Ordiniamo entrambe le stringhe e confrontiamole
