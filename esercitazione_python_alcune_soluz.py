...
def check_S_in_T(S, T):
    ...
    risultato = []
    for carattere in S:
        risultato.append(T.count(carattere))
    return risultato
    '''

    return [T.count(carattere) for carattere in S]
...
def count_sub_string(query, corpo):
    ...
    conteggio = 0
    L = len(query)
    for posizione in range(len(corpo)):
        if query == corpo[posizione:posizione+L]:
            conteggio += 1
    return conteggio
...
def count_sub_string_idx(query, corpo):
    ...
    for posizione in range(len(corpo)):
        if query == corpo[posizione:posizione+L]:
            risultato.append((posizione,posizione+L-1))
    return risultato
    '''
    return [ (posizione,posizione+L-1)  for posizione in range(len(corpo))  
                                        if query == corpo[posizione:posizione+L] ]
...
def check_reverse(S, T):
    ...
    if len(S) != len(T):
        return False
    for posizione in range(len(S)):
        if S[posizione] != T[-posizione-1]:
            return False
    return True
...
def list_multipli(L):
    ...
    for X in L:
        if X%2==0:
            multipli_di_2.append(X)
        if X%5==0:
            multipli_di_5.append(X)
    return multipli_di_2 + multipli_di_5    

    
    '''
    multipli_di_2 = [ X for X in L if X%2==0 ]
    multipli_di_5 = [ X for X in L if X%5==0 ]
    return multipli_di_2 + multipli_di_5
    '''
...
def cumulative_sum(L):
    ...
    for pos in range(len(L)):
        risultato.append(sum(L[:pos+1]))
    return risultato
    '''

    # inefficiente
    return [ sum(L[:pos+1]) for pos in range(len(L)) ]
...
def reshape_by_index1(L, src, dst):
    pass
    # inserisci qui il tuo codice
    quanti = max(dst)+1
    out = [None] * quanti
...
def reshape_by_index2(L, src, dst):
    pass
    # inserisci qui il tuo codice
    quanti = max(dst)+1
    out = [None] * quanti
    for pos, da in enumerate(src):
        a  = dst[pos]
        out[a] = L[da]
    return out
